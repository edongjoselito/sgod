...
$actionHistoryBySubmission = isset($actionHistoryBySubmission) && is_array($actionHistoryBySubmission) ? $actionHistoryBySubmission : array();
    ...
    <div class="content-page"><div class="content"><main class="container-fluid pbei-shell">
        ...
        <?php else: ?>
            <?php foreach ($requirements as $requirement): ?>
				<?php $hasAttachment = trim((string) ($requirement->stored_name ?? '')) !== ''; $submissionStatus = strtoupper(trim((string) ($requirement->submission_status ?? ''))); $isNotApplicable = $submissionStatus === 'NOT APPLICABLE'; $isValidated = $hasAttachment && $submissionStatus === 'VALIDATED'; $isReturned = $hasAttachment && $submissionStatus === 'RETURNED'; ?>
				<section class="card pbei-item" data-requirement-status="<?= $hasAttachment ? 'submitted' : 'pending'; ?>"><div class="card-body p-4">
	                    <div class="d-flex align-items-start justify-content-between mb-3"><div class="d-flex align-items-start"><span class="pbei-order mr-3"><?= (int) $requirement->sort_order; ?></span><div><h5 class="pbei-title mb-1"><?= $esc($requirement->requirement); ?></h5><?php if (trim((string) $requirement->description) !== ''): ?><p class="text-muted mb-0"><?= nl2br($esc($requirement->description)); ?></p><?php endif; ?></div></div><span class="pbei-status <?= ($hasAttachment || $isNotApplicable) ? 'pbei-status--submitted' : 'pbei-status--pending'; ?>"><i class="mdi <?= ($hasAttachment || $isNotApplicable) ? 'mdi-check-circle-outline' : 'mdi-clock-outline'; ?>"></i><?= $isNotApplicable ? 'Not Applicable' : ($hasAttachment ? ($isValidated ? 'Validated' : ($isReturned ? 'Returned to School' : 'For Validation')) : 'Not Submitted'); ?></span></div>
                    <form method="post" action="<?= base_url(); ?>Page/school_pbei_requirement_save" enctype="multipart/form-data">
                        ...
                        <div class="form-row">
								...
								<div class="custom-control custom-checkbox mb-3"><input type="checkbox" class="custom-control-input" id="notApplicable<?= (int) $requirement->id; ?>" name="not_applicable" value="1" <?= $isNotApplicable ? 'checked' : ''; ?> <?= $isValidated ? 'disabled' : ''; ?> onchange="sessionStorage.setItem('pbeiRequirementsScroll', window.scrollY);this.form.submit();"><label class="custom-control-label" for="notApplicable<?= (int) $requirement->id; ?>">Not Applicable</label></div>
                                    <?php if (!$isValidated): ?><label><?= trim((string) $requirement->stored_name) !== '' ? 'Replace PDF' : 'Upload PDF'; ?></label><div class="custom-file"><input type="file" class="custom-file-input" id="requirementPdf<?= (int) $requirement->id; ?>" name="requirement_pdf" accept="application/pdf,.pdf"><label class="custom-file-label" for="requirementPdf<?= (int) $requirement->id; ?>">Choose a PDF file</label></div><small class="form-text text-muted">PDF only, maximum 5 MB. <?= trim((string) $requirement->stored_name) !== '' ? 'Select a new file to replace the current upload.' : ''; ?></small><?php endif; ?>
								<?php if (trim((string) $requirement->stored_name) !== ''): ?><?php $remarkText = $isValidated ? 'Validated' : 'For Validation'; ?><div class="pbei-upload-status"><a class="btn btn-sm btn-outline-primary" target="_blank" rel="noopener" title="Preview uploaded PDF" aria-label="Preview uploaded PDF" href="<?= base_url(); ?>upload/pbei_requirement_submissions/<?= rawurlencode(basename((string) $requirement->stored_name)); ?>"><i class="mdi mdi-eye-outline mr-1"></i>Preview PDF</a><?php if (!$isValidated): ?><button class="btn btn-sm btn-outline-danger" type="submit" formaction="<?= base_url(); ?>Page/school_pbei_requirement_remove_attachment" formmethod="post" onclick="return confirm('Remove this attachment?');"><i class="mdi mdi-delete-outline mr-1"></i>Remove attachment</button><?php endif; ?><span class="pbei-remarks <?= $isValidated ? 'pbei-remarks--validated' : 'pbei-remarks--validation'; ?>"><strong>Remarks:</strong> <?= $esc($remarkText); ?></span></div><?php endif; ?>
	                                <?php if (trim((string) $requirement->division_remarks) !== ''): ?><button type="button" class="btn btn-sm btn-outline-primary mt-2" data-toggle="modal" data-target="#commentsModal<?= (int) $requirement->submission_id; ?>"><i class="mdi mdi-comment-text-outline mr-1"></i>View comments</button><div class="modal fade" id="commentsModal<?= (int) $requirement->submission_id; ?>" tabindex="-1" role="dialog" aria-labelledby="commentsModalLabel<?= (int) $requirement->submission_id; ?>" aria-hidden="true"><div class="modal-dialog modal-dialog-scrollable" role="document"><div class="modal-content"><div class="modal-header"><h5 class="modal-title" id="commentsModalLabel<?= (int) $requirement->submission_id; ?>">Division comments</h5><button type="button" class="close" data-dismiss="modal">&times;</button></div><div class="modal-body"><p class="font-weight-bold mb-3"><?= $esc($requirement->requirement); ?></p><?php foreach (($actionHistoryBySubmission[(int) $requirement->submission_id] ?? array()) as $action): ?><div class="border-bottom pb-3 mb-3"><strong><?= $esc($action->action); ?></strong><br><small class="text-muted"><?= $esc($action->actor_name); ?> · <?= $esc(date('M j, Y g:i A', strtotime($action->created_at))); ?></small><?php if (trim((string) $action->comment) !== ''): ?><div class="mt-2"><?= nl2br($esc($action->comment)); ?></div><?php endif; ?></div><?php endforeach; ?><div class="border-top pt-3"><strong><?= $isReturned ? 'Returned by Division — Action Required:' : "Current validator comment:"; ?></strong><div class="mt-2"><?= nl2br($esc($requirement->division_remarks)); ?></div></div></div><div class="modal-footer"><button type="button" class="btn btn-light" data-dismiss="modal">Close</button></div></div></div></div><?php endif; ?>
                            </div>
	                        ...
	                        <div class="d-flex justify-content-between align-items-center"><span class="pbei-saved"><?php if (!empty($requirement->submitted_at)): ?>Last saved: <?= $esc(date('M j, Y g:i A', strtotime($requirement->submitted_at))); ?><?php endif; ?></span><?php if ($isValidated): ?><span class="text-success font-weight-bold"><i class="mdi mdi-lock-outline mr-1"></i>Validated — editing is locked</span><?php elseif ($isNotApplicable): ?><button class="btn btn-secondary" type="button" disabled><i class="mdi mdi-block-helper mr-1"></i>Not Applicable</button><?php else: ?><button class="btn btn-primary" type="submit"><i class="mdi mdi-content-save-outline mr-1"></i>Save</button><?php endif; ?></div>
                    </form>
                </div></section>
            <?php endforeach; ?>
        <?php endif; ?>
