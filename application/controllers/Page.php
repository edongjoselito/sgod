...
class Page extends CI_Controller
{
	...
	{
		$data['title'] = "Memo Update";
		$id = $this->input->get('id');
		$result['data'] = $this->SGODModel->getMemo($id);
		$this->load->view('sgod_memo_update', $result);
	...
	function memo_attach()
	{
		$data['title'] = "Memo Attachment";
		$id = $this->input->get('id');
		$result['data'] = $this->SGODModel->getMemo($id);
		$this->load->view('sgod_memo_attach', $result);
		...
		if ($this->input->post('submit')) {
			...
			if ($this->upload->do_upload('fileName')) {
				...
				$this->SGODModel->memo_attach();
				$this->session->set_flashdata('success', '1 file uploaded successfully!');
				redirect(base_url() . 'Page/memo_attach?id=' . $id);
			} else {
    ...
    {
        $result['img'] = $this->SGODModel->get_single_table_by_id("id", 'sgod_memo', $param);
        $filename = $result['img']['fileName'];
        $id = $result['img']['id'];
        $this->SGODModel->delete_group($param, $filename, 'memo', "sgod_memo");
	...
	function settings_strand()
	{
		...
		$data['domain'] = $this->SGODModel->get_all('sgod_settings_domain');
	...
	function settings_pias()
	{
		...
		$data['m_title'] = "Add New PIA";
		$data['e_title'] = "Update PIA";
		$data['label'] = "PIA Description";
	...
	function sections()
	{
		...
		if ($this->input->post('submit')) {
			...
			$que = $this->db->query("insert into sgod_sections (sectionName, sectionHead, sectionHeadPosition, secGroup) values('$sectionName','$sectionHead','$sectionHeadPosition','$param')");
			$this->session->set_flashdata('success', ' Added Successfully!');
	...
	{
		$date = date('Y') + 1;
		$result['title'] = "SELECT ANNUAL IMPLEMENTATION PLAN";
		$result['ssa'] = $this->SGODModel->two_cond('sgod_school_allocation', 'schoolID', $this->session->username, 'alloc_year', date('Y') + 1);


		$this->load->view('aip_filter', $result);
	}
	...
	function submit_aip()
	{
		...
		$check = $this->SGODModel->one_cond_row('sgod_aip_submit', 'b_code', $bcode);
		...
		if (empty($check)) {
			...
			$this->session->set_flashdata('danger', 'Already Saved');
			redirect(base_url() . 'Page/aip_action_list');
		}
	}
	...
	function generate_app()
	{
		...
		$result['budget'] = $this->SGODModel->one_cond_row('sgod_school_allocation', 'alloc_batch', $b_code);
		$result['aip_sum'] = $this->SGODModel->aip_budget_sum($fy, $school_id, $b_code);
	...
	function aip_new()
	{
		...
		$result['io'] = $this->SGODModel->get_all('sgod_setting_io');
		$result['ssa'] = $this->SGODModel->two_cond('sgod_school_allocation', 'schoolID', $this->session->username, 'alloc_year', date('Y') + 1);
